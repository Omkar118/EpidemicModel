...
def main():
    import time
    start = time.time()
    with open('/home/retro/Desktop/projectepidemic/pkl/X.pkl', 'rb') as fh: # Load data set
            X = dill.load(fh)
    with open('/home/retro/Desktop/projectepidemic/pkl/y.pkl', 'rb') as fh:
            y = dill.load(fh)
    scaler = Normalizer()
    smote_etomek=SMOTETomek('auto')
    cachedir = mkdtemp()
